...
def deallocate_img(data: Dict[str, any], new_name: str, axis: int = 0) -> List[np.ndarray]:
    """
    Divides image into parts based on given name and assembles it to a new image based on new name
    :param data: Object consisting of name and directory
    :param new_name: Name
    ...
    img: np.ndarray = load_img(directory)
    divisions: Dict[str, List[np.ndarray]] = get_divisions(img, name, axis)
...
def get_divisions(img: np.ndarray, name: str, axis: int = 0) -> Dict[str, List[np.ndarray]]:
    ...
    """
    ret: DefaultDict = defaultdict(list)
...
def assemble_img(new_name: str, locator: Dict[str, List[np.ndarray]], axis: int = 0) -> List[np.ndarray]:
    ...
    if not set(new_name) <= set(locator.keys()):
        new_name: str = ''.join(locator.keys()).lower()
    indices: List[Tuple[any]] = [i for i in it.product(*[list(range(0, len(locator[c]))) for c in new_name])]
